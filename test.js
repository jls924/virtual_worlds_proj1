...
var start = PIXI.Texture.fromImage("StartScreen.png");
...
var endGameMsg = new PIXI.Text("You made it to hyperspace!", { font: 'bold 60px Arial', fill: '#6536d1', align: 'center', stroke: '#FFFFFF', strokeThickness: 6 });
endGameMsg.alpha = 0;
// used http://scottmcdonnell.github.io/pixi-examples/index.html?s=demos&f=text-demo.js&title=Text 

var deadMsg = new PIXI.Text("You died! Refresh browser.", { font: 'bold 60px Arial', fill: '#6536d1', align: 'center', stroke: '#FFFFFF', strokeThickness: 6 });
deadMsg.alpha = 0;
...
function animate() {
	...
	// if hyperspace has been reached
	if (endGameFlag)
	{
		endGameMsg.alpha = 1;
	...
	if (Math.abs(planetPurp2.x - planetPoint.x) <1)
	{
		placePoint();
		collision(collObjects);
	}

	if (Math.abs(planetPurp3.x - planetPoint.x) <1)
	{
		placePoint(); 
		collision(collObjects);
	}
